...
//    reg [40:0] dinParallel; // din converted to Parallel by SIPO
    ...
    SIPO_41bit sipo_inst(.parallel_out(dinParallel_w), .done(SIPO_done), .serial_in(din), .clk(clock), .rst(rst_SIPO), .shift(shift_SIPO));
    PISO_28bit piso_inst(.serial_out(result_wire), .done(PISO_done), .parallel_in(resultParallel), .clk(clock), .rst(rst_PISO), .load(load), .shift(shift_PISO));
...
//                        $display("dinParallel_w[40:33] = %b", dinParallel_w[40:33]);
                ...
                IDLE: begin
                // Serial Input Is not Changing, so there is no need to capture it.
                // 
...
//                        $display("\t PISO result_wire: %b",result_wire);
                    ...
                    if (PISO_done) begin
//                        $display("\n PISO_Complete");
//                        $display("\t Final result_wire: %b",result_wire);
...
module SIPO_41bit(
    ...
    always @(posedge clk) begin
        ...
        if(parallel_out[40:33] == 8'h5A) begin
            ...
            $strobe("CTRL PTRN\n\t parallel_out[40:33] = %h", parallel_out);
            // Remember to Disable shift Externally
        end
    end
endmodule
...
//            $display("--------------- SCARY!!! --------- IT HAPPENED ---------");
////            done <= 1;                  // Mark as done
////            parallel_out <= shift_reg;  // Update parallel output
////            $display("\t shift_reg[FINAL] = %h",shift_reg);
////            $display("\t parallel_out[OLD] = %h",parallel_out);
            
////            shift_reg <= 41'b0; // Potentially Destructive Code
////            bit_counter <= 6'b0; // Potentially Destructive Code
//        end
//        if (rst) begin
//            $display("SIPO RST");
...
//            done <= 0;
//            parallel_out <= 41'b0;
////            $display("\t serial_in[RST] = %b", serial_in);
//        end else if (shift && !done) begin
//            // Shift operation
////            $display("\t shift_reg[OLD] = %h",shift_reg);
////            $display("\t bit_counter[OLD] = %d",bit_counter);
////            $display("\t shift_reg[OLD] = %b",shift_reg);
////            $display("\t serial_in[NEW] = %b", serial_in);
//            shift_reg <= {shift_reg[39:0], serial_in};
...
//                done <= 1;                  // Mark as done
//                parallel_out <= shift_reg;  // Update parallel output
//                $display("\t shift_reg[FINAL] = %b",shift_reg);
//                $display("\t shift_reg[FINAL] = %h",shift_reg);
//                $display("\t parallel_out[OLD] = %h",parallel_out);
...
//            end
//        end else if (!shift) begin
//            $display("STRANGE PLACE...");
//            done <= 0;  // Reset done signal when shifting stops// HERE
//        end
//    end
//endmodule
